...
> ⚠️ **Используете режим Kernel module?** Обязательно обновляйте kernel-модуль
> на хосте (`sudo bash deploy/switch-mode.sh --kernel`) при каждом обновлении
> контейнера Cascade — иначе интерфейсы перестанут стартовать. В первую
> очередь касается установок **до скачка протокола AmneziaWG на версию 3.0
> (2026-07-30)**. См. [Обновление](#-обновление).

## ✨ Возможности

| Модуль | Описание                                                                                                                                                   |
|--------|------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 🔌 **Интерфейсы** | Несколько туннельных интерфейсов WireGuard / AmneziaWG, быстрое создание, импорт `.conf` как аплинк, MSS clamping на интерфейс                             |
| 👥 **Пиры** | Клиентские и S2S-пиры (site-to-site) с QR-кодами, статистикой трафика, ограничением скорости и группировкой по client-group                                |
| 🌐 **Маршрутизация** | Статические маршруты, PBR (Policy-Based Routing), просмотр таблиц ядра, OSPF  в планах                                                                     |
| 🔀 **NAT** | Outbound MASQUERADE / SNAT с поддержкой алиасов + Port Forwarding (DNAT) с привязкой к интерфейсу                                                          |
| 🛡️ **Файрвол** | Правила фильтрации (ACCEPT / DROP / REJECT) + PBR через шлюз                                                                                               |
| 📋 **Алиасы** | 7 типов: host, network, ipset, client-group, group, port, port-group. Client-group — автоматически управляемый ipset по членству пиров                     |
...
| 🔐 **Аутентификация** | Мультипользовательские аккаунты, TOTP 2FA (Google Authenticator), долгосрочные API-токены                                                                  |
...
| 🖥️ **Мультисервер** 🆕 | Управление несколькими роутерами Cascade из одного UI — переключение серверов в сайдбаре, прозрачное проксирование API, поддержка self-signed сертификатов |
| 📊 **Мониторинг** 🆕 | Метрики трафика по интерфейсам в реальном времени, история состояния шлюзов (stacked bar), страница Diagnostics с выбором периода                          |
| ⚡ **Speed Test** 🆕 | Тест скорости между любыми управляемыми серверами через iperf3 — Auto / Tunnel / Internet режим, автоопределение S2S туннеля, история результатов          |
| 🚦 **Rate Limits** 🆕 | Ограничение пропускной способности по группам клиентов через tc HTB (кбит/с вниз/вверх на IP)                                                              |
| 🧙 **Визарды** 🆕 | Пошаговые визарды настройки: Simple Client VPN, Cascade via WireGuard Uplink, Cascade ↔ Cascade S2S interconnect                                           |
...
- ✅ **Port Forwarding (DNAT)** — прозрачный каскад трафика с опциональным source NAT
- ✅ **Импорт .conf как аплинк** — подключить Cascade клиентом к любому WireGuard-серверу и использовать как PBR-шлюз без изменения таблицы маршрутизации
- ✅ **Мониторинг шлюзов** — ICMP ping + HTTP/S-пробы, автоматический fallback при отказе
- ✅ **Мультипользователи + TOTP 2FA** — аккаунты для каждого пользователя с поддержкой Google Authenticator
...
- ✅ 🆕 **Управление несколькими серверами** — управляйте несколькими роутерами Cascade из одной вкладки браузера
- ✅ 🆕 **Встроенный Speed Test** — iperf3 между любыми управляемыми серверами, автоопределение S2S туннеля, история
- ✅ 🆕 **Мониторинг трафика** — метрики по интерфейсам и история состояния шлюзов с выбором периода
- ✅ 🆕 **Визарды настройки** — пошаговые визарды для Uplink VPN и S2S interconnect; автоматически создают интерфейсы, алиасы, шлюзы, PBR-правила и NAT
...
> ⚠️ Пересинхронизируйте kernel-модуль (`deploy/switch-mode.sh --kernel`) при
> каждом обновлении Cascade — в первую очередь важно, если вы ставили Cascade
> **до скачка протокола AmneziaWG на версию 3.0 (2026-07-30)**, поскольку
> модуль не обновляется сам и может разойтись с версией в Docker-образе.
...
| 2b | **Сетевой режим Docker** — Host (по умолчанию) или Bridge (диапазон портов для публикации) |
| 3 | Установка Docker CE |
| 4 | sysctl: `ip_forward`, UDP-буферы |
| 4b | TCP-тюнинг: BBR congestion control, FQ scheduler, `rp_filter` |
| 5a | Генерация decoy-видео через ffmpeg (60 с шума — выглядит как настоящий стрим) |
...
| 10 | Проверка: health-check Cascade + Caddy, вывод итогов |
...
Откройте в браузере — при первом запуске отображается форма **Создать первого пользователя** (авторизация не требуется, пока нет ни одного аккаунта).
После создания аккаунта включите **TOTP 2FA** в Settings → Users для дополнительной защиты.
...
Сетевой режим Docker отображается отдельным бэйджем (серый = HOST, жёлтый = BRIDGE, красный = NONE).
...
| `NETWORK_MODE` | `host` | `host` или `bridge` — сетевой режим Docker |
| `BRIDGE_PORT_RANGE` | *(только bridge)* | Диапазон UDP-портов для публикации WireGuard в режиме bridge (например `51831-65535`) |
...
- **Мультипользовательские аккаунты** — у каждого пользователя свои логин и пароль
- **TOTP 2FA** — Google Authenticator / Authy (включается для каждого пользователя в Settings → Users)
- **API-токены** — долгосрочные bearer-токены для скриптов; обходят TOTP; отзываемые
...
> ⚠️ **Используете режим Kernel module?** Docker-образ следует за актуальной
> линией протокола AmneziaWG (`:latest`), а kernel-модуль на хосте сам по
> себе не обновляется. В первую очередь это касается установок **до скачка
> протокола AmneziaWG на версию 3.0 (2026-07-30)** — модуль остался на старой
> линии, а образ переехал на 3.0.x, из-за чего netlink-формат `awg setconf`
> перестаёт совпадать с тем, что ожидает старый модуль. Если версии
> разойдутся — интерфейсы перестанут стартовать с ошибкой `Unable to modify
> interface: Invalid argument`. Всегда пересинхронизируйте kernel-модуль
> вместе с обновлением контейнера:
> ```bash
> sudo bash deploy/switch-mode.sh --kernel
> ```
> Userspace-режим этой проблемы не касается — он не зависит от kernel-модуля на хосте.

### Режим host network (по умолчанию)
