...
func List() ([]*Remote, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&r.ID, &r.Name, &r.URL, &createdAt); err != nil {
...
func Get(id string) (*Remote, error) {
	...
	var createdAt string
	err := db.DB().QueryRow(`
		SELECT id, name, url, token, created_at
		FROM remotes WHERE id = ?
	`, id).Scan(&r.ID, &r.Name, &r.URL, &r.Token, &createdAt)
	if err != nil {
...
func Add(name, url, token string) (*Remote, error) {
	if name == "" {
	...
	_, err := db.DB().Exec(`
		INSERT INTO remotes (id, name, url, token, created_at)
		VALUES (?, ?, ?, ?, ?)
	`, r.ID, r.Name, r.URL, token, r.CreatedAt.Format("2006-01-02 15:04:05"))
	if err != nil {
