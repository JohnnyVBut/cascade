...
## ⚠️ ОБЯЗАТЕЛЬНЫЙ WORKFLOW ДЛЯ КАЖДОГО КОММИТА

**Порядок строго обязателен — нарушение недопустимо:**

0. **Git checkpoint** — перед крупными изменениями (новая фича, рефакторинг, security fix):
   ```bash
   git tag checkpoint/<описание> && git push origin checkpoint/<описание>
   ```
1. **Реализация** — написать код
2. **Тесты** — запустить агента `tester`:
   - написать тесты для нового кода (каждая фича должна быть покрыта)
   - запустить полный test suite (`go test ./internal/... -count=1 -timeout 120s`)
   - все тесты должны быть зелёными
3. **Code Review** — запустить агента `code-reviewer`:
   - проверить изменённые файлы
   - исправить все CRITICAL и HIGH проблемы
   - MEDIUM/LOW — по ситуации
4. **Коммит** — только если тесты зелёные И код ревьюер не нашёл критичных проблем

**Коммит без прохождения шагов 2 и 3 — ЗАПРЕЩЁН.**
