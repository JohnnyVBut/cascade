...
Пример: Сервер A (сеть 10.100.0.0/24) ↔ Сервер B (сеть 10.100.1.0/24)
...
4. **Disable Routes: ✓** (маршруты будем прописывать вручную)
...
1. Создайте интерфейс на Сервере B (Address: `10.100.1.1/24`, Disable Routes: ✓)
...
#### Шаг 6 — Добавить маршруты

На каждом сервере добавьте статический маршрут к подсети партнёра:

- **Сервер A → Routing → Static Routes → + Add**
  - Destination: `10.100.1.0/24`
  - Via: `10.100.1.1` (IP интерфейса Сервера B в туннеле)
  - Dev: `wg10`
