...
Скачивает архив `cascade-backup-YYYYMMDD-HHMMSS.tar.gz` содержащий:
- `awg.db` — вся конфигурация: интерфейсы, пиры, правила NAT/Firewall, алиасы, пользователи, шлюзы
- `ipsets/` — содержимое всех ipset-алиасов (save-файлы)

> **Что НЕ входит в бэкап:** `.env` (ADMIN_PATH, переменные окружения), `docker-compose.yml`, TLS-сертификаты. Их нужно перенести отдельно.

#### Полное восстановление системы

**Settings → System Backup → Restore Backup**

Выберите `.tar.gz` файл бэкапа. Система:
1. Заменит `awg.db` и `ipsets/` данными из архива
2. Перезапустит контейнер (требуется `restart: always` в `docker-compose.yml`)
3. Страница перезагрузится через ~4 секунды
...
3. На новом сервере: установить Docker, склонировать репо, собрать (`./build.sh`)
4. Запустить: `docker compose up -d`
5. Восстановить: **Settings → System Backup → Restore Backup**
6. Обновить DNS / firewall-правила на новый IP
7. Сообщить пользователям новый endpoint (IP сервера изменился)
