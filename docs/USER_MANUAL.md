...
Обычная настройка клиентского VPN-сервера с одним важным отличием: в поле **Host** (endpoint
для клиентских конфигов) нужно указать **публичный IP Сервера A**, а не самого Сервера B.

1. **Interfaces → + New Interface**
   - Protocol: AmneziaWG 2.0 (или WireGuard 1.0)
   - Address: `10.8.0.1/24`
   - Listen Port: `51820`
   - **Host (Endpoint)**: `<публичный IP Сервера A>` ← клиенты будут подключаться сюда

2. Запустить интерфейс — Cascade автоматически добавит MASQUERADE для клиентского трафика
   в интернет (правило из PostUp). Дополнительная настройка NAT не нужна.

3. Создать пиров — в скачанных конфигах endpoint будет `<IP Сервера A>:51820`.
