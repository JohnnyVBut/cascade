#!/usr/bin/env bash
# =============================================================================
# Cascade — one-shot fresh-VPS bootstrap: Kernel module mode, Host network
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/JohnnyVBut/cascade/master/deploy/quickstart-kernel.sh | sudo bash
#   curl -fsSL https://raw.githubusercontent.com/JohnnyVBut/cascade/master/deploy/quickstart-kernel.sh | sudo bash -s -- --staging
#   # or, after cloning the repo yourself:
#   sudo bash deploy/quickstart-kernel.sh [--staging]
#
...
export AWG_USERSPACE_IMPL=kernel
export NETWORK_MODE=host

echo "→ Running setup.sh --yes ${STAGING_ARGS[*]} (Kernel module mode, Host network)"
