...
# Patch docker-compose.go.yml with current values
COMPOSE_FILE="$REPO_DIR/docker-compose.go.yml"
sed -i "s|WG_HOST=.*|WG_HOST=${WG_HOST}|"       "$COMPOSE_FILE"
sed -i "s|BIND_ADDR=.*|BIND_ADDR=${BIND_ADDR}|" "$COMPOSE_FILE"
sed -i "s|- PORT=.*|- PORT=${CASCADE_PORT}|"        "$COMPOSE_FILE"
# Clear PASSWORD_HASH — first admin is created via UI (open mode)
