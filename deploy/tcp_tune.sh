...
# ── fq на физические интерфейсы (не виртуальные/туннельные) ────────────────
...
for iface_path in /sys/class/net/*/; do
    ...
    # Пропускаем loopback
    [[ "$iface" == "lo" ]] && continue
    # Пропускаем WireGuard и tunnel-интерфейсы — fq на них бессмысленен:
    # трафик уже зашифрован в UDP, реальная очередь на физическом интерфейсе
    case "$iface" in
        # WG/tunnel: fq бессмысленен — трафик уже зашифрован в UDP
